...
func SetupChezmoi() {
	...
	if _, err := exec.LookPath(ChezmoiDependencies); err != nil {
		...
		if err := CreateFile(ChezmoiConfigFile); err != nil {
			...
			chezmoiSourceDir, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "sourceDir")), chezmoiSourceDir)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			chezmoiAutoCommit, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "autoCommit")), chezmoiAutoCommit)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			chezmoiAutoPush, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "autoPush")), chezmoiAutoPush)
...
func SetupCobra() {
	...
	if _, err := exec.LookPath(CobraDependencies); err != nil {
		...
		if err := CreateFile(CobraConfigFile); err != nil {
			...
			color.Printf(askItemTitleFormat, 2, " ", SuccessText("-"), LightText("Config"))
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			cobraAuthorName, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "authorName")), cobraAuthorName)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			cobraAuthorEmail, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "authorEmail")), cobraAuthorEmail)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			cobraLicense, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "license")), cobraLicense)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			cobraUseViper, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "useViper")), cobraUseViper)
...
func SetupGit() {
	...
	if _, err := exec.LookPath(GitDependencies); err != nil {
		...
		if err := CreateFile(GitConfigFile); err != nil {
			...
			color.Printf(askItemTitleFormat, 2, " ", SuccessText("-"), LightText("Config"))
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			hostname, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "[user].name")), hostname)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			gitUserEmail, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "[user].email")), gitUserEmail)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			gitCoreEditor, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "[core].editor")), gitCoreEditor)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			gitCoreAutoCRLF, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "[core].autocrlf")), gitCoreAutoCRLF)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			gitDiffExternal, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "[diff].external")), gitDiffExternal)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			gitMergeTool, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "[merge].tool")), gitMergeTool)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			gitColorUI, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "[color].ui")), gitColorUI)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			gitPullRebase, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "[pull].rebase")), gitPullRebase)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			gitFilterLfsClean, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "[filter \"lfs\"].clean")), gitFilterLfsClean)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			gitFilterLfsSmudge, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "[filter \"lfs\"].smudge")), gitFilterLfsSmudge)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			gitFilterLfsProcess, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "[filter \"lfs\"].process")), gitFilterLfsProcess)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			gitFilterLfsRequired, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "[filter \"lfs\"].required")), gitFilterLfsRequired)
...
func SetupGolang() {
	...
	if _, err := exec.LookPath(GolangDependencies); err != nil {
		...
		if err := CreateFile(GolangConfigFile); err != nil {
			...
			color.Printf(askItemTitleFormat, 2, " ", SuccessText("-"), LightText("Config"))
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			golangGO111MODULE, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "GO111MODULE")), golangGO111MODULE)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			golangGOPATH, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "GOPATH")), golangGOPATH)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			golangGOCACHE, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "GOCACHE")), golangGOCACHE)
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			golangGOMODCACHE, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "GOMODCACHE")), golangGOMODCACHE)
...
func SetupPip() {
	...
	if _, err := exec.LookPath(PipDependencies); err != nil {
		...
		if err := CreateFile(PipConfigFile); err != nil {
			...
			color.Printf(askItemTitleFormat, 2, " ", SuccessText("-"), LightText("Config"))
			color.Printf(askItemsFormat, 4, " ", SuccessText("-"))
			pipIndexUrl, _ = GetUserInput(QuestionText(color.Sprintf(InputTips, "index-url")), pipIndexUrl)
